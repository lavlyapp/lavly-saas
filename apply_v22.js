// Node.js script to execute the fixed V22 SQL patch
...
const sql = `
-- =========================================================================
-- V22 FINAL (CORRIGIDO): CLASSIFICAÇÃO NATIVA TEF + PROTEÇÃO DE FUSO E RBAC
-- Esta versão corrige o status de 'A Classificar' lendo diretamente 'tipo_cartao'
-- =========================================================================

-- 1. FUNÇÃO FINANCEIRA (AJUSTADA)
...
BEGIN
    ...
    SELECT json_build_object(
        ...
        'paymentStats', COALESCE((
            ...
            FROM (
                SELECT forma_pagamento as metodo, SUM(total_valor) as valor
                FROM tmp_mv_sales
                WHERE forma_pagamento != 'TEF'
                ...
                SELECT 
                    CASE 
                        WHEN tipo_cartao ILIKE '%Crédito%' THEN 'Crédito'
                        WHEN tipo_cartao ILIKE '%Débito%' THEN 'Débito'
                        ELSE 'Crédito/Débito'
                    END as metodo, 
                    SUM(valor) as valor
                FROM sales
                WHERE forma_pagamento = 'TEF'
                  AND data >= (start_dia::timestamp at time zone 'UTC' + interval '3 hours')
                  AND data <= (end_dia::timestamp at time zone 'UTC' + interval '26 hours 59 minutes 59 seconds')
                  AND loja = ANY(v_accessible_stores)
                  AND (p_store = 'Todas' OR loja = p_store)
                GROUP BY 1
            ) m
...
async function apply() {
  const { error } = await s.rpc('execute_sql_query', { query: sql });
  if (error) {
    if (error.code === 'PGRST202') {
       // execute_sql_query missing, we must use a migration endpoint or direct postgres 
       console.log('MUST RUN MANUALLY OR USE A ROUTE MIGRATION!');
    }
  } else {
