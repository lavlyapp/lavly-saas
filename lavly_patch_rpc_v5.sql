CREATE OR REPLACE FUNCTION get_financial_dashboard_metrics(p_store text DEFAULT 'Todas', p_start_date timestamp with time zone DEFAULT NULL, p_end_date timestamp with time zone DEFAULT NULL)
RETURNS json
LANGUAGE plpgsql
SECURITY INVOKER
AS $$
...
BEGIN
    -- 1. Grab accessible stores into memory array to prevent Postgres RLS nested loop explosion on materialized views
    SELECT array_agg(name) INTO v_accessible_stores FROM stores;
    
    -- Se p_start_date is null, significa allTime: desde o início de 2020.
    start_dia := COALESCE(p_start_date::date, '2020-01-01'::date);
    end_dia := COALESCE(p_end_date::date, CURRENT_DATE);

    -- CREATE TEMP TABLES PRE-FILTERED BY THE STATIC ARRAY
    -- Sales
    ...
    -- Machines
    CREATE TEMP TABLE tmp_mv_machines ON COMMIT DROP AS
    SELECT * FROM mv_machines_states
    WHERE loja = ANY(v_accessible_stores)
      AND (p_store = 'Todas' OR loja = p_store);
      
    -- Customers
    CREATE TEMP TABLE tmp_mv_customers ON COMMIT DROP AS
    SELECT * FROM mv_customers_stats
    WHERE loja = ANY(v_accessible_stores)
      AND (p_store = 'Todas' OR loja = p_store);
    ...
    SELECT json_build_object(
        'period', json_build_object(
            ...
            'endDate', p_end_date,
            'uniqueCustomers', (SELECT COUNT(*) FROM tmp_mv_customers)
        ),
