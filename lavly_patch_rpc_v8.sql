...
-- Criando a função completamente otimizada com as colunas reais das Materialized Views
...
BEGIN
    ...
    SELECT json_build_object(
        ...
        'basketsMetrics', json_build_object(
            ...
            'totalWashes', COALESCE((SELECT SUM(qtd_ciclos) FROM tmp_mv_orders WHERE upper(service) LIKE '%LAVAGEM%'), 0),
            'totalDries', COALESCE((SELECT SUM(qtd_ciclos) FROM tmp_mv_orders WHERE upper(service) LIKE '%SECAGEM%'), 0),
            'totalOthers', COALESCE((SELECT SUM(qtd_ciclos) FROM tmp_mv_orders WHERE upper(service) NOT LIKE '%LAVAGEM%' AND upper(service) NOT LIKE '%SECAGEM%'), 0),
