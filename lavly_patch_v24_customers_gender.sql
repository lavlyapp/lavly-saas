-- =========================================================================
-- V24: CRM CUSTOM DATES & NATIVE VMPAY GENDER FIX
-- Atualiza `get_crm_backend_metrics` para extrair o `gender` direto
-- da tabela `customers`, alimentada pela lógica de cadastros da VMPay,
-- descartando a dependência do frontend inferir o gênero por sufixos.
-- =========================================================================
...
BEGIN
    ...
    -- Usamos datas seguras de fallback para evitar falhas com nulos
    ...
    -- [NOVO] Adicionado LEFT JOIN com 'customers' para pescar o gender preenchido na base 
    CREATE TEMP TABLE tmp_period_profiles ON COMMIT DROP AS
    SELECT 
        s.cliente AS name,
        MAX(s.telefone) AS phone,
        MAX(COALESCE(c.gender, 'U')) AS gender,
    ...
    FROM sales s
    LEFT JOIN customers c ON s.cliente = c.name
