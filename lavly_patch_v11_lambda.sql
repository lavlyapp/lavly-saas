...
-- Arquitetura Lambda: Une o histórico rápido (Materialized View) com os dados de "Hoje" ao vivo (Tabelas Brutas)
...
BEGIN
    ...
    -- SALES: Speed Layer + Batch Layer
    CREATE TEMP TABLE tmp_mv_sales ON COMMIT DROP AS
    SELECT * FROM mv_sales_daily
    WHERE dia >= start_dia AND dia < CURRENT_DATE
      AND loja = ANY(v_accessible_stores)
      AND (p_store = 'Todas' OR loja = p_store)
    UNION ALL
    SELECT data::date as dia, loja, forma_pagamento, sum(valor) as total_valor, count(*) as qtd_vendas, count(distinct customer_id) as qtd_clientes
    FROM sales
    ...
    -- ORDERS: Speed Layer + Batch Layer
    ...
    WHERE dia >= start_dia AND dia < CURRENT_DATE
      ...
      AND (p_store = 'Todas' OR loja = p_store)
    UNION ALL
    SELECT data::date as dia, loja, service, count(*) as qtd_ciclos
    FROM orders
    WHERE data >= CURRENT_DATE AND data <= end_dia AND data >= start_dia
      AND loja = ANY(v_accessible_stores)
      AND (p_store = 'Todas' OR loja = p_store)
