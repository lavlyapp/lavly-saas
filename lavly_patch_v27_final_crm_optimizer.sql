-- =========================================================================
-- LAVLY SaaS - PATCH V27 (FINAL CRM OPTIMIZER)
-- Corrige o erro "column c.vmpay_id does not exist", substituindo 
-- por "c.updated_at DESC" para pescar a atualização mais recente.
-- =========================================================================
...
BEGIN
    ...
    SELECT 
        ps.*,
        COALESCE(
           (SELECT c.gender FROM customers c WHERE c.name = ps.name AND c.gender IS NOT NULL AND c.gender != 'U' ORDER BY c.updated_at DESC LIMIT 1), 
           'U'
        ) AS gender
    ...
    SELECT 
        gs.*,
        COALESCE(
           (SELECT c.gender FROM customers c WHERE c.name = gs.name AND c.gender IS NOT NULL AND c.gender != 'U' ORDER BY c.updated_at DESC LIMIT 1), 
           'U'
        ) AS gender
