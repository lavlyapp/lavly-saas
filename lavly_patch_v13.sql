-- 1. Reverte a função de métricas para a versão Materializada Pura (Mais rápida do Universo, Zero Timeouts)
...
-- 2. Cria uma função de Sistema para Atualizar as Views em Tempo Real
CREATE OR REPLACE FUNCTION refresh_dashboard_materialized_views()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER -- Garante que qualquer atendente possa atualizar o cache local das Views
AS $$
BEGIN
    BEGIN
        REFRESH MATERIALIZED VIEW CONCURRENTLY mv_sales_daily;
        REFRESH MATERIALIZED VIEW CONCURRENTLY mv_orders_daily;
        REFRESH MATERIALIZED VIEW CONCURRENTLY mv_customers_stats;
        REFRESH MATERIALIZED VIEW CONCURRENTLY mv_machines_states;
    EXCEPTION WHEN OTHERS THEN
        -- Se não tiver unique index para Concurrently, faz um refresh hard limit
        REFRESH MATERIALIZED VIEW mv_sales_daily;
        REFRESH MATERIALIZED VIEW mv_orders_daily;
        REFRESH MATERIALIZED VIEW mv_customers_stats;
        REFRESH MATERIALIZED VIEW mv_machines_states;
    END;
END;
$$;
