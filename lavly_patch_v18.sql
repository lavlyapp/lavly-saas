-- =========================================================================
-- V18: CORREÇÃO TICKET MÉDIO E CESTOS (CONTAGEM EXATA DE CLIENTES)
-- =========================================================================
-- Resolve o problema da contagem de clientes superestimada nas visões materializadas.
-- O "Ticket Médio" e "Cestos por Cliente" precisam do COUNT(DISTINCT) exato considerando o período inteiro.
...
BEGIN
    ...
    -- Ajuste de fuso horário seguro para o corte diário (Materialized Views)
    ...
    -- Tabela temporária baseada nas visões materializadas ultrarrápidas
    ...
    -- *NOVO*: Obter a contagem EXATA de clientes únicos, consultando a tabela base (indexada por data)
    -- Isso garante que um cliente que comprou em 3 dias diferentes no mesmo mês não conte como 3.
    ...
    WHERE (p_start_date IS NULL OR data >= p_start_date::timestamp with time zone)
      ...
      AND loja = ANY(v_accessible_stores)
      AND (p_store = 'Todas' OR loja = p_store)
      AND (produto != 'BRLD' OR produto IS NULL); -- Opcional, mantido de versões prévias de filtro
    ...
    SELECT json_build_object(
        ...
        'salesMetrics', json_build_object(
            ...
            -- Ticket Médio Correto: Faturamento BRUTO dividido pelo NÚMERO EXATO DE CLIENTES.
            'averageTicket', CASE
